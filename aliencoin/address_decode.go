...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := aliencoin_addrdec.ALC_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = aliencoin_addrdec.ALC_testnetPrivateWIFCompressed
	}

	wif, _ := aliencoin_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	aliencoin_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := aliencoin_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := aliencoin_addrdec.ALC_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = aliencoin_addrdec.ALC_testnetAddressP2SH
	}
	...
	address, _ := aliencoin_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := aliencoin_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
...
func (decoder *addressDecoder) ScriptPubKeyToBech32Address(scriptPubKey []byte) (string, error) {
	...
	cfg := aliencoin_addrdec.ALC_mainnetAddressBech32V0
	if decoder.wm.Config.IsTestNet {
		cfg = aliencoin_addrdec.ALC_testnetAddressBech32V0
	}
