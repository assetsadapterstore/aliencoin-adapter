...
import (
	...
	"github.com/blocktree/go-owcdrivers/aliencoinTransaction"
...
func (decoder *TransactionDecoder) SignBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			//签名交易
			/////////交易单哈希签名
			signature, err := aliencoinTransaction.SignTransaction(txHash, keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		...
		sigPub     = make([]aliencoinTransaction.SigPub, 0)
	...
	for accountID, keySignatures := range rawTx.Signatures {
		...
		for _, keySignature := range keySignatures {
			...
			signaturePubkey := aliencoinTransaction.SigPub{
				Signature: signature,
				Pubkey:    pubkey,
			}
	...
	pass, signedTrans, err := aliencoinTransaction.VerifyAndCombineTransaction(emptyTrans, sigPub)
	if pass {
...
func (decoder *TransactionDecoder) createBTCRawTransaction(
	...
	var (
		err              error
		vins             = make([]aliencoinTransaction.Vin, 0)
		vouts            = make([]aliencoinTransaction.Vout, 0)
		totalSend        = decimal.New(0, 0)
		destinations     = make([]string, 0)
		accountTotalSent = decimal.Zero
	...
	for _, utxo := range usedUTXO {
		in := aliencoinTransaction.Vin{utxo.TxID, uint32(utxo.Vout), utxo.ScriptPubKey}
		vins = append(vins, in)
	...
	for to, amount := range to {
		...
		out := aliencoinTransaction.Vout{to, intAmount}
	...
	/////////构建空交易单
	emptyTrans, transHash, err := aliencoinTransaction.CreateEmptyTransactionAndHash(vins, vouts, lockTime)
	if err != nil {
